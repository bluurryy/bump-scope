...
- Fix soundness bug where `Bump` incorrectly implements `Send` when its base allocator `A` doesn't

## [2.3.2] - 2026-07-09

### Security

- Fix documentation of `BumpAllocatorCore` with respect to the guarantees for `grow*`, `shrink` and `deallocate`. Relying on those guarantees was able to cause use-after-free bugs when calling the methods with an allocation from an ancestor scope. See [GHSA-xhx6-q96x-pf9c](https://github.com/bluurryy/bump-scope/security/advisories/GHSA-xhx6-q96x-pf9c).

## [2.3.1] - 2026-06-23

### Added

- Add `SECURITY.md` to document that only the latest release receives security updates and to describe how to report vulnerabilities

### Changed

- Improve documentation of `alloc_uninit` and `split_off`
...
- Don't shrink `BumpVec` / `BumpString` with a `WithoutShrink` allocator (via not shrinking in `BumpAllocator::shrink_slice`)
...
<!-- next-url -->
[Unreleased]: https://github.com/bluurryy/bump-scope/compare/v2.3.3...HEAD
[2.3.3]: https://github.com/bluurryy/bump-scope/releases/tag/v2.3.3
[2.3.2]: https://github.com/bluurryy/bump-scope/releases/tag/v2.3.2
[2.3.1]: https://github.com/bluurryy/bump-scope/releases/tag/v2.3.1
[2.3.0]: https://github.com/bluurryy/bump-scope/releases/tag/v2.3.0
