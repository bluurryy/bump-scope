...
use common::{MALLOC_OVERHEAD, either_way};

use crate::common::{BumpNoMinSize, BumpScopeNoMinSize};
...
fn mut_bump_vec_drop<const UP: bool>() {
    use bump_scope::MutBumpVec;

    use crate::common::{BumpNoMinSize, ChunkHeader, MALLOC_OVERHEAD, OVERHEAD};

    const SIZE: usize = 32;
    assert_eq!(size_of::<ChunkHeader>(), SIZE);
...
fn scope_by_guards<const UP: bool>() {
...

    {
        ...
        let child_scope = child_guard.scope();
...
fn test_drop_allocator() {
    ...
    let bump: Bump<_> = Bump::with_size_in(512, allocator.clone());
    drop(bump);
    assert_eq!(drop_count.get(), 1);

    let bump: Bump<_> = Bump::with_size_in(512, allocator);
    bump.reserve(1024);
    drop(bump);
...
fn generic_bump_scope() {
    fn foo(bump: &mut impl BumpAllocator) {
