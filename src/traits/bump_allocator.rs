...
pub trait BumpAllocator: MutBumpAllocatorTyped + Sized {
    ...
    fn scope_guard(&mut self) -> BumpScopeGuard<'_, Self::Allocator, Self::Settings> {
        BumpScopeGuard::new(self.as_mut_scope().as_mut_bump())
    }
    ...
    /// assert!(bump.stats().get_current_chunk().unwrap().bump_position().is_aligned_to(16));
    ///
    /// // allocate one byte
    /// bump.alloc(1u8);
    ///
    ...
    /// assert!(bump.stats().get_current_chunk().unwrap().bump_position().addr().get() % 2 == 1);
