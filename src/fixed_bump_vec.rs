...
/// This is like [`vec!`](alloc_crate::vec!) but allocates inside a bump allocator, returning a [`FixedBumpVec`].
///
...
///
/// - Create an empty [`FixedBumpVec`]:
/// ```
/// # use bump_scope::{ Bump, fixed_bump_vec, FixedBumpVec };
/// # let bump: Bump = Bump::new();
/// let vec: FixedBumpVec<i32> = fixed_bump_vec![in &bump];
/// assert!(vec.is_empty());
/// ```
///
/// - Create a [`FixedBumpVec`] containing a given list of elements:
///
/// ```
/// # use bump_scope::{ Bump, fixed_bump_vec };
/// # let bump: Bump = Bump::new();
/// let vec = fixed_bump_vec![in &bump; 1, 2, 3];
/// assert_eq!(vec[0], 1);
/// assert_eq!(vec[1], 2);
/// assert_eq!(vec[2], 3);
/// ```
///
/// - Create a [`FixedBumpVec`] from a given element and size:
///
/// ```
/// # use bump_scope::{ Bump, fixed_bump_vec };
/// # let bump: Bump = Bump::new();
/// let vec = fixed_bump_vec![in &bump; 1; 3];
/// assert_eq!(vec, [1, 1, 1]);
/// ```
///
...
#[macro_export]
macro_rules! fixed_bump_vec {
    [in $bump:expr] => {{
        let _ = $bump;
        $crate::FixedBumpVec::new()
    }};
    [in $bump:expr; $($values:expr),* $(,)?] => {
        $crate::FixedBumpVec::from_array_in([$($values),*], $bump)
    };
    [in $bump:expr; $value:expr; $count:expr] => {
        $crate::FixedBumpVec::from_elem_in($value, $count, $bump)
    };
    [try in $bump:expr] => {{
        let _ = $bump;
        Ok::<_, $crate::alloc::AllocError>($crate::FixedBumpVec::new())
    }};
    [try in $bump:expr; $($values:expr),* $(,)?] => {
        $crate::FixedBumpVec::try_from_array_in([$($values),*], $bump)
    };
    [try in $bump:expr; $value:expr; $count:expr] => {
        $crate::FixedBumpVec::try_from_elem_in($value, $count, $bump)
    };
}
