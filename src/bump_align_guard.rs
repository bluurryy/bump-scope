use crate::{MinimumAlignment, MutBumpScope, SupportedMinimumAlignment, align_pos};
...
/// The `MutBumpScope` is in an invalid state when the bump pointer alignment does not match `MIN_ALIGN`.
...
{
    pub(crate) scope: &'b mut MutBumpScope<'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED, DEALLOCATES>,
}
...
{
    #[inline(always)]
    pub(crate) fn new(scope: &'b mut MutBumpScope<'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED, DEALLOCATES>) -> Self {
        Self { scope }
    }
}
