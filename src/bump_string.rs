...
use crate::{error_behavior_generic_methods, polyfill, BumpBox, BumpScopeRef, BumpVec, ErrorBehavior, FromUtf8Error};
    ...
    BS: BumpScopeRef<'a>,
{
    ...
    unsafe fn insert_bytes<E: ErrorBehavior>(&mut self, idx: usize, bytes: &[u8]) -> Result<(), E> {
        let len = self.len();
        let amt = bytes.len();
