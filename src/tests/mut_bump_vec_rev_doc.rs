...
use crate::{mut_vec_rev, Bump, MutVecRev};
...
fn new<const UP: bool>() {
    ...
    let vec: MutVecRev<i32, Global, 1, UP> = mut_vec_rev![in bump];
    assert!(vec.is_empty());
}
...
fn capacity<const UP: bool>() {
    ...
    let vec: MutVecRev<i32, Global, 1, UP> = MutVecRev::with_capacity_in(2048, &mut bump);
