...
{
    bumps: Mutex<Vec<Bump<A, MIN_ALIGN, UP, CONST_NEW>>>,
    allocator: A,
}

impl<A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool> Default for BumpPool<A, MIN_ALIGN, UP, CONST_NEW>
where
...
#[cfg(feature = "alloc")]
impl<const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool> BumpPool<Global, MIN_ALIGN, UP, CONST_NEW>
where
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
{
...
impl<A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool> BumpPool<A, MIN_ALIGN, UP, CONST_NEW>
where
    A: Allocator + Clone,
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
{
    ...
    pub fn bumps(&mut self) -> &mut Vec<Bump<A, MIN_ALIGN, UP, CONST_NEW>> {
    ...
    pub fn get(&self) -> BumpPoolGuard<A, MIN_ALIGN, UP, CONST_NEW> {
    ...
    pub fn try_get(&self) -> Result<BumpPoolGuard<A, MIN_ALIGN, UP, CONST_NEW>, AllocError> {
        let bump = self.bumps.lock().unwrap_or_else(PoisonError::into_inner).pop();
...
#[derive(Debug)]
pub struct BumpPoolGuard<'a, A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool>
where
    A: Allocator + Clone,
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
{
    bump: ManuallyDrop<Bump<A, MIN_ALIGN, UP, CONST_NEW>>,
    pool: &'a BumpPool<A, MIN_ALIGN, UP, CONST_NEW>,
}

impl<'a, A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool> Deref
    for BumpPoolGuard<'a, A, MIN_ALIGN, UP, CONST_NEW>
where
    A: Allocator + Clone,
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
{
    type Target = BumpScope<'a, A, MIN_ALIGN, UP, CONST_NEW>;
...
impl<'a, A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool> DerefMut
    for BumpPoolGuard<'a, A, MIN_ALIGN, UP, CONST_NEW>
where
    A: Allocator + Clone,
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
...
impl<'a, A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool> Drop
    for BumpPoolGuard<'a, A, MIN_ALIGN, UP, CONST_NEW>
where
    A: Allocator + Clone,
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
...
unsafe fn transmute_lifetime<'from, 'to, 'b, A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool>(
    scope: &'b BumpScope<'from, A, MIN_ALIGN, UP, CONST_NEW>,
) -> &'b BumpScope<'to, A, MIN_ALIGN, UP, CONST_NEW> {
    mem::transmute(scope)
}

// This exists as a "safer" transmute that only transmutes the `'a` lifetime parameter.
#[allow(clippy::needless_lifetimes)]
unsafe fn transmute_lifetime_mut<'from, 'to, 'b, A, const MIN_ALIGN: usize, const UP: bool, const CONST_NEW: bool>(
    scope: &'b mut BumpScope<'from, A, MIN_ALIGN, UP, CONST_NEW>,
) -> &'b mut BumpScope<'to, A, MIN_ALIGN, UP, CONST_NEW> {
    mem::transmute(scope)
}
