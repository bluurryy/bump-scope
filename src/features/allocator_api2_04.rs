...
pub struct AllocatorApi2V04Compat<A: ?Sized>(pub A);

impl<A: ?Sized> AllocatorApi2V04Compat<A> {
    #[inline(always)]
    #[expect(missing_docs)]
...
unsafe impl<A: ?Sized + Allocator> CrateAllocator for AllocatorApi2V04Compat<A> {
    #[inline(always)]
    fn allocate(&self, layout: Layout) -> Result<NonNull<[u8]>, CrateAllocError> {
...
unsafe impl<A: ?Sized + CrateAllocator> Allocator for AllocatorApi2V04Compat<A> {
    #[inline(always)]
    fn allocate(&self, layout: Layout) -> Result<NonNull<[u8]>, AllocError> {
