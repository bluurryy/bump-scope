#![allow(clippy::match_same_arms, clippy::needless_lifetimes, clippy::uninlined_format_args)]
...
/// }
/// ```
pub fn utf8_chunks(source: &[u8]) -> Utf8Chunks<'_> {
    Utf8Chunks { source }
}
...
impl fmt::Debug for Debug<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        for chunk in utf8_chunks(self.0) {
            // Valid part.
...
impl<'a> Iterator for Utf8Chunks<'a> {
    ...
    fn next(&mut self) -> Option<Utf8Chunk<'a>> {
        ...
        let (inspected, remaining) = unsafe { polyfill::slice::split_at_unchecked(self.source, i) };
        self.source = remaining;

        // SAFETY: `valid_up_to <= i` because it is only ever assigned via
        // `valid_up_to = i` and `i` only increases.
        let (valid, invalid) = unsafe { polyfill::slice::split_at_unchecked(inspected, valid_up_to) };
