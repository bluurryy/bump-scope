...
use crate::{
    error_behavior_generic_methods, polyfill, Box, BumpScope, ErrorBehavior, FromUtf8Error, GuaranteedAllocatedStats,
    MinimumAlignment, MutVec, Stats, SupportedMinimumAlignment,
};

/// This is like [`alloc::format!`] but allocates inside a *mutable* `Bump` or `BumpScope`, returning a [`MutString`].
///
...
///
/// ```
/// # use bump_scope::{ Bump, mut_format };
/// # let mut bump: Bump = Bump::new();
/// #
/// let greeting = "Hello";
/// let mut string = mut_format!(in bump, "{greeting} world!");
/// string.push_str(" How are you?");
///
/// assert_eq!(string, "Hello world! How are you?");
/// ```
#[macro_export]
macro_rules! mut_format {
    (in $bump:expr) => {{
        $crate::MutString::new_in($bump.as_mut_scope())
    }};
    (in $bump:expr, $($arg:tt)*) => {{
        let mut string = $crate::MutString::new_in($bump.as_mut_scope());
    ...
    (try in $bump:expr) => {{
        Ok::<_, $crate::allocator_api2::alloc::AllocError>($crate::MutString::new_in($bump.as_mut_scope()))
    }};
...
/// A type like [`String`](crate::String), optimized for a `&mut Bump(BumpScope)`.
///
/// When you are done building the string, you can turn it into a `&str` with [`into_str`].
///
/// # Examples
///
/// You can create a `MutString` from [a literal string][`&str`] with [`MutString::from_str_in`]:
///
/// [`into_str`]: MutString::into_str
///
/// ```
/// # use bump_scope::{ Bump, MutString };
/// # let mut bump: Bump = Bump::new();
/// let hello = MutString::from_str_in("Hello, world!", &mut bump);
/// ```
///
...
/// # use bump_scope::{ Bump, MutString };
/// # let mut bump: Bump = Bump::new();
/// let mut hello = MutString::from_str_in("Hello, ", &mut bump);
///
/// hello.push('w');
/// hello.push_str("orld!");
///
...
///
/// [`push`]: MutString::push
/// [`push_str`]: MutString::push_str
///
/// If you have a vector of UTF-8 bytes, you can create a `MutString` from it with
/// the [`from_utf8`] method:
///
/// ```
/// # use bump_scope::{ Bump, MutString, mut_vec };
/// # let mut bump: Bump = Bump::new();
/// // some bytes, in a vector
/// let sparkle_heart = mut_vec![in bump; 240, 159, 146, 150];
///
/// // We know these bytes are valid, so we'll use `unwrap()`.
/// let sparkle_heart = MutString::from_utf8(sparkle_heart).unwrap();
///
/// assert_eq!("💖", sparkle_heart);
/// ```
///
/// [`&str`]: prim@str "&str"
/// [`from_utf8`]: MutString::from_utf8
...
> {
    vec: MutVec<'b, 'a, u8, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>,
}

impl<'b, 'a: 'b, const MIN_ALIGN: usize, const UP: bool, const GUARANTEED_ALLOCATED: bool, A>
    MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>
where
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
    A: Allocator + Clone,
{
    /// Constructs a new empty `MutString`.
    ///
    /// The vector will not allocate until elements are pushed onto it.
    pub fn new_in(bump: impl Into<&'b mut BumpScope<'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>>) -> Self {
        Self {
            vec: MutVec::new_in(bump),
        }
    }

    error_behavior_generic_methods! {
        /// Constructs a new empty `MutString` with at least the specified capacity
        ...
        fn generic_with_capacity_in(capacity: usize, bump: impl Into<&'b mut BumpScope<'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>>) -> Self {
            Ok(Self { vec: MutVec::generic_with_capacity_in(capacity, bump.into())? } )
        }

        /// Constructs a new `MutString` from a `&str`.
        impl
...
impl<'b, 'a: 'b, const MIN_ALIGN: usize, const UP: bool, const GUARANTEED_ALLOCATED: bool, A>
    MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>
{
    /// Returns this `MutString`'s capacity, in bytes.
    #[must_use]
    #[inline(always)]
    ...
    /// Returns the length of this `MutString`, in bytes, not [`char`]s or
    ...
    /// Returns `true` if this `MutString` has a length of zero, and `false` otherwise.
    #[must_use]
    #[inline(always)]
    ...
    /// Truncates this `MutString`, removing all contents.
    ///
    /// While this means the `MutString` will have a length of zero, it does not
    /// touch its capacity.
    ///
    /// # Examples
    ///
    ...
    /// # let mut bump: Bump = Bump::new();
    /// #
    /// let mut s = MutString::from_str_in("foo", &mut bump);
    ///
    /// s.clear();
    ///
    /// assert!(s.is_empty());
    ...
    /// Converts a bump allocated vector of bytes to a `MutString`.
    ///
    /// A string ([`MutString`]) is made of bytes ([`u8`]), and a vector of bytes
    /// ([`MutVec<u8>`]) is made of bytes, so this function converts between the
    /// two. Not all byte slices are valid `MutString`s, however: `MutString`
    ...
    /// sake.
    ///
    /// If you need a [`&str`] instead of a `MutString`, consider
    /// [`str::from_utf8`].
    ///
    ...
    /// [`MutVec<u8>`]: MutVec
    /// [`&str`]: prim@str "&str"
    /// [`into_bytes`]: MutString::into_bytes
    pub fn from_utf8(
        vec: MutVec<'b, 'a, u8, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>,
    ) -> Result<Self, FromUtf8Error<MutVec<'b, 'a, u8, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>>> {
    ...
    /// Converts a `MutString` into a `MutVec<u8>`.
    ///
    /// This consumes the `MutString`, so we do not need to copy its contents.
    ///
    /// # Examples
    ///
    /// ```
    /// # use bump_scope::{ Bump, MutString };
    /// # let mut bump: Bump = Bump::new();
    /// let mut s = MutString::new_in(&mut bump);
    /// s.push_str("hello");
    ...
    pub fn into_bytes(self) -> MutVec<'b, 'a, u8, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED> {
        self.vec
    }

    /// Returns a byte slice of this `MutString`'s contents.
    #[must_use]
    #[inline(always)]
    ...
    /// Extracts a string slice containing the entire `MutString`.
    #[must_use]
    #[inline(always)]
    ...
    /// Converts a `MutString` into a mutable string slice.
    #[must_use]
    #[inline(always)]
    ...
    /// Returns a mutable reference to the contents of this `MutString`.
    ///
    /// # Safety
    ///
    /// This function is unsafe because the returned `&mut Vec` allows writing
    /// bytes which are not valid UTF-8. If this constraint is violated, using
    /// the original `MutString` after dropping the `&mut Vec` may violate memory
    /// safety, as `MutString`s must be valid UTF-8.
    #[must_use]
    #[inline(always)]
    pub unsafe fn as_mut_vec(&mut self) -> &mut MutVec<'b, 'a, u8, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED> {
        &mut self.vec
    }
    ...
    /// # let mut bump: Bump = Bump::new();
    /// #
    /// let mut s = MutString::from_str_in("abç", &mut bump);
    ///
...
{
    error_behavior_generic_methods! {
        /// Appends the given [`char`] to the end of this `MutString`.
        impl
        for pub fn push
        for pub fn try_push
        ...
        /// Appends a given string slice onto the end of this `MutString`.
        impl
        for pub fn push_str
        ...
        /// Inserts a character into this `MutString` at a byte position.
        ///
        /// This is an *O*(*n*) operation as it requires copying every element in the
        /// buffer.
        do panics
        ...
        /// # let mut bump: Bump = Bump::new();
        /// #
        /// let mut s = MutString::with_capacity_in(3, &mut bump);
        ///
        /// s.insert(0, 'f');
        /// s.insert(1, 'o');
        /// s.insert(2, 'o');
        ...
        /// Inserts a string slice into this `MutString` at a byte position.
        ///
        /// This is an *O*(*n*) operation as it requires copying every element in the
        /// buffer.
        do panics
        /// Panics if `idx` is larger than the `MutString`'s length, or if it does not
        /// lie on a [`char`] boundary.
        do examples
        /// ```
        /// # use bump_scope::{ Bump, MutString };
        /// # let mut bump: Bump = Bump::new();
        /// #
        /// let mut s = MutString::from_str_in("bar", &mut bump);
        ///
        /// s.insert_str(0, "foo");
        ///
        /// assert_eq!("foobar", s);
        ...
        /// # let mut bump: Bump = Bump::new();
        /// #
        /// let mut string = MutString::from_str_in("abcde", &mut bump);
        ///
    ...
    /// Converts a `MutString` into a `Box<str>`.
    #[must_use]
    #[inline(always)]
    ...
    A: Allocator + Clone,
{
    ...
    #[doc = crate::doc_fn_stats_greedy!(MutString)]
    #[must_use]
    #[inline(always)]
...
impl<'b, 'a: 'b, A, const MIN_ALIGN: usize, const UP: bool> MutString<'b, 'a, A, MIN_ALIGN, UP, true>
where
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
    A: Allocator + Clone,
{
...
impl<'b, 'a: 'b, const MIN_ALIGN: usize, const UP: bool, const GUARANTEED_ALLOCATED: bool, A> fmt::Write
    for MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>
where
    MinimumAlignment<MIN_ALIGN>: SupportedMinimumAlignment,
    A: Allocator + Clone,
{
...
macro_rules! impl_partial_eq {
    (
        ...
        $(
            $(#[$attr])*
            impl<'b, 'a, const MIN_ALIGN: usize, const UP: bool, const GUARANTEED_ALLOCATED: bool, A> PartialEq<$string_like> for MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED> {
                #[inline]
            ...
            $(#[$attr])*
            impl<'b, 'a, const MIN_ALIGN: usize, const UP: bool, const GUARANTEED_ALLOCATED: bool, A> PartialEq<MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>> for $string_like {
                #[inline]
                fn eq(&self, other: &MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>) -> bool {
                    <str as PartialEq>::eq(self, other)
                }

                #[inline]
                fn ne(&self, other: &MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>) -> bool {
                    <str as PartialEq>::ne(self, other)
                }
            }
        )*
...
impl<'b, 'a, const MIN_ALIGN: usize, const UP: bool, const GUARANTEED_ALLOCATED: bool, A>
    From<MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>> for alloc::string::String
{
    #[inline]
    fn from(value: MutString<'b, 'a, A, MIN_ALIGN, UP, GUARANTEED_ALLOCATED>) -> Self {
        value.as_str().into()
    }
}
