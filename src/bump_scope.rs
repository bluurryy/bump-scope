    ...
    A: BaseAllocator<GUARANTEED_ALLOCATED>,
{
        ...
        unsafe {
            if UP {
                ...
                if is_last {
                    ...
                    let old_addr = nonnull::addr(old_ptr);
                    let new_addr = crate::bump_down(old_addr, additional_size, T::ALIGN.max(MIN_ALIGN));
    ...
    pub(crate) fn generic_shrink_vec_to_fit<T>(&self, fixed: &mut FixedBumpVec<'a, T>) {
        ...
        unsafe {
            ...
            if UP {
                ...
                let new_addr = crate::bump_down(old_addr_old_end, new_size, T::ALIGN.max(MIN_ALIGN));
                let new_addr = NonZeroUsize::new_unchecked(new_addr);
