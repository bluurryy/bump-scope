...
macro_rules! mut_bump_vec {
    ...
    [in $bump:expr; $($values:expr),* $(,)?] => {
        $crate::__mut_bump_vec_panic_on_alloc![in $bump; $($values),*]
    };
...
#[macro_export]
#[cfg(feature = "panic-on-alloc")]
macro_rules! __mut_bump_vec_panic_on_alloc {
    [in $bump:expr; $($values:expr),* $(,)?] => {
...
macro_rules! __mut_bump_vec_panic_on_alloc {
    [in $bump:expr; $($values:expr),* $(,)?] => {
        compile_error!(concat!(
            "the potentially panicking api of `mut_bump_vec!` is not available\n",
...
impl<T, A: MutBumpAllocatorTyped> MutBumpVec<T, A> {
        ...
        I: IntoIterator<Item = T>,
    {
        ...
        let mut vector = match iterator.next() {
            None => return Ok(MutBumpVec::new_in(allocator)),
            Some(element) => {
                let (lower, _) = iterator.size_hint();
                let initial_capacity = cmp::max(const { min_non_zero_cap(T::SIZE) }, lower.saturating_add(1));
                let mut vector = MutBumpVec::generic_with_capacity_in(initial_capacity, allocator)?;
                unsafe {
