...
impl<T, A> MutBumpVecRev<T, A> {
    ...
    /// # use bump_scope::{Bump, MutBumpVecRev};
    /// # let mut bump: Bump = Bump::new();
    /// let mut vec = MutBumpVecRev::with_capacity_in(3, &mut bump);
    /// vec.append([2, 1]);
...
impl<T, A: MutBumpAllocatorTyped> MutBumpVecRev<T, A> {
    ...
    /// # use bump_scope::{mut_bump_vec_rev, Bump};
    /// # let mut bump: Bump = Bump::new();
    /// let mut vec = mut_bump_vec_rev![in &mut bump; 'b', 'd'];
    /// vec.insert(1, 'c');
    /// assert_eq!(vec, ['b', 'c', 'd']);
    /// vec.insert(0, 'a');
    ...
    /// # use bump_scope::{mut_bump_vec_rev, Bump};
    /// # let mut bump: Bump = Bump::new();
    /// let mut vec = mut_bump_vec_rev![try in &mut bump; 'b', 'd']?;
    /// vec.try_insert(1, 'c')?;
    /// assert_eq!(vec, ['b', 'c', 'd']);
    /// vec.try_insert(0, 'a')?;
    /// assert_eq!(vec, ['a', 'b', 'c', 'd']);
    ...
    /// # use bump_scope::{ Bump, mut_bump_vec_rev };
    /// # let mut bump: Bump = Bump::new();
    /// let mut vec = mut_bump_vec_rev![in &mut bump; 4];
    /// vec.extend_from_slice_copy(&[1, 2, 3]);
    ...
    /// # use bump_scope::{ Bump, mut_bump_vec_rev };
    /// # let mut bump: Bump = Bump::new();
    /// let mut vec = mut_bump_vec_rev![in &mut bump; String::from("c")];
    /// vec.extend_from_slice_clone(&[String::from("a"), String::from("b")]);
    /// assert_eq!(vec, ["a", "b", "c"]);
    /// ```
    #[inline(always)]
    #[cfg(feature = "panic-on-alloc")]
