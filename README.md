...
Memory for the most recent allocation *can* be reclaimed. You can also use [scopes, checkpoints](#scopes-and-checkpoints) and [`reset`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.reset) to reclaim memory.
...
```
The fact that the bump allocator allocates ever larger chunks and [`reset`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.reset) only keeps around the largest one means that after a few iterations, every bump allocation
...
- Drop is always called for allocated values unless explicitly [leaked](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpBox.html#method.leak) or [forgotten](https://doc.rust-lang.org/core/mem/fn.forget.html).
  - `alloc*` methods return a [`BumpBox<T>`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpBox.html) which owns and drops `T`. Types that don't need dropping can be turned into references with [`into_ref`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpBox.html#method.into_ref) and [`into_mut`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpBox.html#method.into_mut).
- You can allocate a slice from *any* `Iterator` with [`alloc_iter`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.alloc_iter).
...
You can create a new scope either with a [`scoped`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.scoped) closure or with a [`scope_guard`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.scope_guard):
```rust
use bump_scope::Bump;
...
assert_eq!(bump.stats().allocated(), 0);
```
You can also use the unsafe [`checkpoint`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.checkpoint) api
...
- [`split_off`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpVec.html#method.split_off) —  splits the collection in place without allocation; the parameter is a range instead of a single index
- [`retain`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpVec.html#method.retain) —  takes a closure with a `&mut T` parameter like `Vec::retain_mut`

##### New features
- [`append`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpVec.html#method.append) —  allows appending all kinds of owned slice types like `[T; N]`, `Box<[T]>`, `Vec<T>`, `vec::Drain<T>` etc.
- [`map`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpVec.html#method.map) —  maps the elements, potentially reusing the existing allocation
- [`map_in_place`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.BumpVec.html#method.map_in_place) —  maps the elements without allocation, failing to compile if not possible
...
This allows you to [bump allocate collections](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#collections).
...
- **`allocator-api2-02`** — Makes `Bump(Scope)` implement `allocator_api2` version `0.2`'s `Allocator` and
  ...
  [`AllocatorApi2V02Compat`](https://docs.rs/bump-scope/1.2.1/bump_scope/alloc/compat/struct.AllocatorApi2V02Compat.html).
- **`allocator-api2-03`** — Makes `Bump(Scope)` implement `allocator_api2` version `0.3`'s `Allocator` and
  makes it possible to use an `allocator_api2::alloc::Allocator` as a base allocator via
  [`AllocatorApi2V03Compat`](https://docs.rs/bump-scope/1.2.1/bump_scope/alloc/compat/struct.AllocatorApi2V03Compat.html).
...
- **`nightly-allocator-api`** — Makes `Bump(Scope)` implement `alloc`'s `Allocator` and
  ...
  [`AllocatorNightlyCompat`](https://docs.rs/bump-scope/1.2.1/bump_scope/alloc/compat/struct.AllocatorNightlyCompat.html).
...
- **`nightly-clone-to-uninit`** — Adds [`alloc_clone`](https://docs.rs/bump-scope/1.2.1/bump_scope/struct.Bump.html#method.alloc_clone) method to `Bump(Scope)`.
