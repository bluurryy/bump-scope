...
Memory for the most recent allocation *can* be reclaimed. You can also use [scopes, checkpoints](#scopes-and-checkpoints) and [`reset`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.reset) to reclaim memory.
...
```
The fact that the bump allocator allocates ever larger chunks and [`reset`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.reset) only keeps around the largest one means that after a few iterations, every bump allocation
...
- Drop is always called for allocated values unless explicitly [leaked](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpBox.html#method.leak) or [forgotten](https://doc.rust-lang.org/core/mem/fn.forget.html).
  - `alloc*` methods return a [`BumpBox<T>`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpBox.html) which owns and drops `T`. Types that don't need dropping can be turned into references with [`into_ref`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpBox.html#method.into_ref) and [`into_mut`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpBox.html#method.into_mut).
- You can allocate a slice from *any* `Iterator` with [`alloc_iter`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.alloc_iter).
...
Have a look at the documentation of [`Bump`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html) for a method overview.
...
You can create a new scope either with a [`scoped`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.scoped) closure or with a [`scope_guard`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.scope_guard):
```rust
use bump_scope::Bump;
...
## Collections
`bump-scope` provides bump allocated variants of `Vec` and `String` called [`BumpVec`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpVec.html) and [`BumpString`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpString.html).
They are also available in the following variants:
- [`Fixed*`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.FixedBumpVec.html) for fixed capacity collections
- [`Mut*`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.MutBumpVec.html) for collections optimized for a mutable bump allocator

##### API changes
The collections are designed to have the same api as their std counterparts with these exceptions:
- [`split_off`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpVec.html#method.split_off) —  splits the collection in place without allocation; the parameter is a range instead of a single index
- [`retain`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpVec.html#method.retain) —  takes a closure with a `&mut T` parameter like `Vec::retain_mut`

##### New features
- [`append`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpVec.html#method.append) —  allows appending all kinds of owned slice types like `[T; N]`, `Box<[T]>`, `Vec<T>`, `vec::Drain<T>` etc.
- [`map`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpVec.html#method.map) —  maps the elements, potentially reusing the existing allocation
- [`map_in_place`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpVec.html#method.map_in_place) —  maps the elements without allocation
- conversions between the regular collections, their `Fixed*` variants and `BumpBox<[T]>` / `BumpBox<str>`

## Parallel Allocation
[`Bump`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html) is `!Sync` which means it can't be shared between threads.

To bump allocate in parallel you can use a [`BumpPool`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.BumpPool.html).

## Allocator API
`Bump` and `BumpScope` implement `bump-scope`'s own [`Allocator`](https://docs.rs/bump-scope/0.17.4/bump_scope/alloc/trait.Allocator.html) trait and with the
...
This allows you to [bump allocate collections](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#collections).
...
You can wrap a bump allocator in a type that makes `deallocate` and `shrink` a no-op using [`WithoutDealloc`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.WithoutDealloc.html) and [`WithoutShrink`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.WithoutShrink.html).
```rust
...
- **`allocator-api2-02`** — Makes `Bump(Scope)` implement `allocator_api2` version `0.2`'s `Allocator` and
  ...
  [`AllocatorApiV02Compat`](https://docs.rs/bump-scope/0.17.4/bump_scope/alloc/compat/struct.AllocatorApi2V02Compat.html).
- **`allocator-api2-03`** — Makes `Bump(Scope)` implement `allocator_api2` version `0.3`'s `Allocator` and
  makes it possible to use an `allocator_api2::alloc::Allocator` as a base allocator via
  [`AllocatorApiV03Compat`](https://docs.rs/bump-scope/0.17.4/bump_scope/alloc/compat/struct.AllocatorApi2V03Compat.html).
...
- **`nightly-allocator-api`** — Makes `Bump(Scope)` implement `alloc`'s `Allocator` and
  ...
  [`AllocatorNightlyCompat`](https://docs.rs/bump-scope/0.17.4/bump_scope/alloc/compat/struct.AllocatorNightlyCompat.html).
...
- **`nightly-coerce-unsized`** — Makes `BumpBox<T>` implement [`CoerceUnsized`](https://doc.rust-lang.org/core/ops/unsize/trait.CoerceUnsized.html).
  ...
  You can unsize a `BumpBox` in stable without this feature using [`unsize_bump_box`](https://docs.rs/bump-scope/0.17.4/bump_scope/macro.unsize_bump_box.html).
...
This benefits collections as well as <code>[alloc_iter](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.alloc_iter)([_mut](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.alloc_iter_mut))</code> and <code>[alloc_fmt](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.alloc_fmt)([_mut](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.alloc_fmt_mut))</code>
with the exception of [`MutBumpVecRev`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.MutBumpVecRev.html) and [`alloc_iter_mut_rev`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.alloc_iter_mut_rev) which
...
This is usually the case unless it was created with [`Bump::unallocated`](https://docs.rs/bump-scope/0.17.4/bump_scope/struct.Bump.html#method.unallocated).
